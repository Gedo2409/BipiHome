<div class="fixed-top">
	<nav class="navbar navbar-expand-lg bg-azul py-3 justify-content-between fixed-top navbar-light">

		{{-- <div> --}}
		<a class="navbar-brand" href="{{ route('front.index') }}">
			<svg version="1.1" id="logo_nav" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 389 138" style="enable-background:new 0 0 389 138;" xml:space="preserve">
				<g>
					<g>
		...
		<ul class="nav collapse navbar-collapse justify-content-end" id="collapsed">

			<li class="nav-item">
				<a href="{{ route('front.acerca') }}" class="btn btn-link text-white">Acerca de BiPi HOME</a>
			</li>
			@if (Auth::guest())
			{{-- <li class="nav-item">
					<a href="{{ route('front.precios') }}" class="btn btn-link text-white">Precios</a>
			</li> --}}
			@else
			...
			@endif
			@endif
			<li class="nav-item dorpdown">
				@if (Auth::guest())
				<button type="button" class="btn btn-link text-white" data-toggle="modal" data-target="#modalSesion"><a href="{{ url('/login') }}" class="text-white">Iniciar Sesión</a></button>
				<button type="button" class="btn btn-link text-white" data-toggle="modal" data-target="#modalSesion"><a href="{{ url('/register') }}" class="text-white">Registrarse</a></button>
				@else
				<a href="{{ route('back.index') }}" class="btn btn-link text-white"> @if(Auth::user()->hasRole('customer')) Perfil @else Panel @endif</a>
				<button type="button" class="btn btn-link text-white" data-toggle="modal" data-target="#modalSesion"><a href="{{ route('logout') }}" class="text-white" onclick="event.preventDefault();
